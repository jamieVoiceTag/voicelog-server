...
app.post("/refresh", async function(req, res) {
  try {
    ...
    if (error || !data || !data.session) return res.status(401).json({ error: "Session expired" });
...
app.post("/transcribe", upload.single("audio"), async function(req, res) {
  try {
    ...
    var prompt = 'Analyse this voice note and return ONLY valid JSON, no markdown:\n\n"' + transcript + '"\n\nReturn: {"tags":["tag1","tag2"],"priority":"high|medium|low","actions":["action1"]}\n\n' + tagInstruction + ' Write any action items in the SAME LANGUAGE as the voice note. Priority: high=time-sensitive, medium=important, low=general — keep the priority value as the English word high, medium, or low. Actions: concrete to-dos or empty array.';
