...
// notes has RLS requiring auth.uid() = user_id. The shared `supabase` client
// above has no per-user identity attached, so RLS checks on it are
// unreliable. This creates a request-scoped client carrying the caller's own
// token, so Postgres can correctly verify auth.uid() for that request.
function notesClientFor(token) {
...
app.get("/subscription", async function(req, res) {
  try {
    ...
    var planInfo = PLANS[plan] || PLANS.free;
    var noteCount = await getNoteCount(user.id, notesClientFor(token));
    var { data: settings } = await supabase
      .from("user_settings")
      .select("daily_email")
      .eq("user_id", user.id)
...
app.post("/transcribe", upload.single("audio"), async function(req, res) {
  try {
    ...
    var prompt = 'Analyse this voice note and return ONLY valid JSON, no markdown:\n\n"' + transcript + '"\n\n' +
      'Return exactly this shape: {"tags":["tag1","tag2"],"priority":"high|medium|low","actions":["action1"],"event":null}\n\n' +
    ...
    catch(e) { analysis = { tags: ["note"], priority: "medium", actions: [], event: null }; }
